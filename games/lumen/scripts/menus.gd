...
func _build_title() -> Control:
	...
	box.add_child(UI.label(
		"Tap a mirror to turn it. Get every ring lit in the colour it is asking for.\n\nTwo colours reaching the same ring mix — red and green make yellow, and nothing else will do.",
...
func _set_panel(which: Control) -> void:
	for p in [_title, _cleared, _shop]:
		p.visible = p == which
