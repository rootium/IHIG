...
## Not fully opaque, so the live court behind the menus stays faintly visible.
const DIM := Color(0.035, 0.04, 0.075, 0.88)
...
func show_game_over(reason: String, points: int, dunks: int, gold: int, best_combo: int,
		earned: int) -> void:
	_over_head.text = reason
	_over_lines.text = "%d points\n%d dunks\nbest run of %d\n%d gold collected\n\n+%d credits" % [
		points, dunks, best_combo, gold, earned]
	_set_panel(_over)
	_refresh_wallet()
...
func _build_title() -> Control:
	...
	box.add_child(_label("DUNK\nRUSH", 70, Color(0.97, 0.97, 1.0)))
	box.add_child(_spacer(8))
	box.add_child(_label("Drag back to aim, release to jump.\nThe dotted line is exactly where you land.\n\nHold in the air to hang. Beat the shot clock.",
		24, Color(0.68, 0.72, 0.86)))
	box.add_child(_spacer(20))

	_best_label = _label("", 28, Color(0.86, 0.89, 1.0))
	box.add_child(_best_label)
	box.add_child(_wallet_label())
	box.add_child(_spacer(24))

	var play := _button("TIP OFF", true)
...
func _build_pause() -> Control:
	var panel := _panel()
	var box := _column(panel)
	box.add_child(_label("TIMEOUT", 56, Color(0.97, 0.97, 1.0)))
	box.add_child(_spacer(24))
...
func _build_over() -> Control:
	...
	_over_lines = _label("", 30, Color(0.86, 0.89, 1.0))
	box.add_child(_over_lines)
	box.add_child(_wallet_label())
	box.add_child(_spacer(24))

	var retry := _button("RUN IT BACK", true)
...
func _build_shop() -> Control:
	...
	box.add_child(_label("SHOP", 46, Color(0.97, 0.97, 1.0)))
	box.add_child(_wallet_label())
	...
	box.add_child(tabs)
	for pair in [["baller", "BALLERS"], ["arena", "ARENAS"]]:
...
func _rebuild_shop() -> void:
	...
	for child in _shop_list.get_children():
		child.queue_free()
	var items: Array[Dictionary] = Skins.BALLERS if _tab == "baller" else Skins.ARENAS
	for item in items:
...
func _shop_row(item: Dictionary) -> Control:
	...
	if _tab == "baller":
		prev.draw.connect(_draw_baller_preview.bind(prev, item))
	else:
		prev.draw.connect(_draw_arena_preview.bind(prev, item))
	h.add_child(prev)

	var text := VBoxContainer.new()
	text.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	text.add_child(_label(str(item.name), 29, Color(0.95, 0.96, 1.0), HORIZONTAL_ALIGNMENT_LEFT))
	text.add_child(_label(str(item.blurb), 19, Color(0.65, 0.69, 0.82), HORIZONTAL_ALIGNMENT_LEFT))
	h.add_child(text)
...
func _action_button(item: Dictionary) -> Button:
	...
	var equipped := id == (SaveData.baller_id if _tab == "baller" else SaveData.arena_id)
...
func _draw_baller_preview(c: Control, item: Dictionary) -> void:
	var mid := c.size * 0.5
	c.draw_circle(mid, 36.0, Color(1, 1, 1, 0.05))
	var jersey: Color = item.get("jersey", Color.WHITE)
	var flesh: Color = item.get("skin", BD.C_SKIN)
	# A little figure with the ball raised, the same pose as a live dunk.
	c.draw_line(mid + Vector2(0, 16), mid + Vector2(0, -6), jersey, 15.0, true)
	c.draw_line(mid + Vector2(-7, 16), mid + Vector2(-11, 30), jersey.darkened(0.25), 6.0, true)
	c.draw_line(mid + Vector2(7, 16), mid + Vector2(11, 30), jersey.darkened(0.25), 6.0, true)
	c.draw_line(mid + Vector2(0, -4), mid + Vector2(15, -20), flesh, 5.0, true)
	c.draw_circle(mid + Vector2(0, -15), 8.0, flesh)
	c.draw_circle(mid + Vector2(19, -25), 8.0, item.get("ball", BD.C_BALL))


func _draw_arena_preview(c: Control, item: Dictionary) -> void:
	var mid := c.size * 0.5
	c.draw_rect(Rect2(mid - Vector2(44, 36), Vector2(88, 72)), item.get("bg", Color.BLACK))
	var board: Color = item.get("board", BD.C_BOARD)
	c.draw_rect(Rect2(mid + Vector2(10, -26), Vector2(6, 30)), board)
	c.draw_line(mid + Vector2(-16, 2), mid + Vector2(10, 2), item.get("rim", BD.C_RIM), 4.0)
	for i in 5:
		c.draw_circle(mid + Vector2(-38 + i * 8, -24), 3.0, item.get("crowd", Color.GRAY))
	c.draw_circle(mid + Vector2(-28, 22), 5.0, item.get("gold", BD.C_GOLD))
...
## `primary` picks the filled treatment used for the one action a screen most
## wants you to take; everything else is a quieter outline.
func _button(text: String, primary: bool = false) -> Button:
	var b := Button.new()
	b.text = text
	b.custom_minimum_size = Vector2(0, 88)
	b.add_theme_font_size_override("font_size", 30)
	var base := BD.C_RIM if primary else Color(1, 1, 1, 0.10)
	var fg := Color(0.06, 0.05, 0.04) if primary else Color(0.93, 0.95, 1.0)
...
func _refresh_wallet() -> void:
	...
	if _best_label != null:
		_best_label.text = "Best: %d points" % SaveData.best_score
